...
std::vector<std::string> dataProjSetUp() {
    ...
    std::sort(theData.begin(), theData.end(), [](shared_ptr<demogData> first, shared_ptr<demogData> second) {return first->getMedIncome() > second->getMedIncome();});

    for (auto &countyData : theData) {
        cout << stripCounty(countyData->getName()) << endl;
        countyNames.push_back(stripCounty(countyData->getName()));
    }
...
int main ( int argc , char** argv)
{
    ...
    try {
        ...
        std::string theStatesExp1, theStatesExp2, theStatesExp3, theStatesExp4, theStatesExp5, theStatesExp6, theStatesExp7, theStatesExp8, theStatesExp9, theStatesExp10;
        int count = round(theStates.size()/10.0);
        int i = 0;
        for (auto state : theStates) {
           
            if (i < count ) {
                theStatesExp1 += "[NAME] = \'" + state + "\'";
                if (i != count-1)
                    theStatesExp1 += " or ";
            }else if (i < count*2) {
                theStatesExp2 += "[NAME] = \'" + state + "\'";
                if (i != 2*count-1)
                    theStatesExp2 += " or ";
            
            }else if (i < count*3) {
                theStatesExp3 += "[NAME] = \'" + state + "\'";
                if (i != 3*count-1)
                    theStatesExp3 += " or ";
            
            }else if (i < count*4) {
                theStatesExp4 += "[NAME] = \'" + state + "\'";
                if (i != 4*count-1)
                    theStatesExp4 += " or ";
            
            }else if (i < count*5) {
                theStatesExp5 += "[NAME] = \'" + state + "\'";
                if (i != 5*count-1)
                    theStatesExp5 += " or ";
            
            }else if (i < count*6) {
                theStatesExp6 += "[NAME] = \'" + state + "\'";
                if (i != 6*count-1)
                    theStatesExp6 += " or ";
            
            }else if (i < count*7) {
                theStatesExp7 += "[NAME] = \'" + state + "\'";
                if (i != 7*count-1)
                    theStatesExp7 += " or ";
            } 
            else if (i < count*8) {
                theStatesExp8 += "[NAME] = \'" + state + "\'";
                if (i != 8*count-1)
                    theStatesExp8 += " or ";
            }
            else if (i < count*9) {
                theStatesExp9 += "[NAME] = \'" + state + "\'";
                if (i != 9*count-1)
                    theStatesExp9 += " or ";
            }else {
                theStatesExp10 += "[NAME] = \'" + state + "\'";
                if (i != 10*count-1 && i !=theStates.size()-1)
                    theStatesExp10 += " or ";
            }
            i++;
        }
        //DEBUG can remove
        std::cout << "the expression 1: " << theStatesExp1 << std::endl << endl;
        std::cout << "the expression 2: " << theStatesExp2 << std::endl << endl;
        std::cout << "the expression 3: " << theStatesExp3 << std::endl << endl;
        std::cout << "the expression 4: " << theStatesExp4 << std::endl << endl;
        std::cout << "the expression 5: " << theStatesExp5 << std::endl << endl;
        std::cout << "the expression 6: " << theStatesExp6 << std::endl << endl;
        std::cout << "the expression 7: " << theStatesExp7 << std::endl << endl;
        std::cout << "the expression 8: " << theStatesExp8 << std::endl << endl;
        std::cout << "the expression 9: " << theStatesExp9 << std::endl << endl;
        std::cout << "the expression 10: " << theStatesExp10 << std::endl << endl;
        ...
        feature_type_style nineTenth_style;
        {
            rule nineTenth_rule;
            nineTenth_rule.set_filter(parse_expression(theStatesExp10)); 
            {
                polygon_symbolizer poly_sym;
                put(poly_sym, keys::fill, color(91, 80, 235));
                nineTenth_rule.append(std::move(poly_sym));
            }
            nineTenth_style.add_rule(std::move(nineTenth_rule));
        }
        m.insert_style("nineTen", std::move(nineTenth_style));
        feature_type_style tenTen_style;
        {
            rule tenTen_rule;
            tenTen_rule.set_filter(parse_expression(theStatesExp9)); 
            {
                polygon_symbolizer poly_sym;
                put(poly_sym, keys::fill, color(95, 166, 245));
                tenTen_rule.append(std::move(poly_sym));
            }
            tenTen_style.add_rule(std::move(tenTen_rule));
        }
        m.insert_style("tenTen", std::move(tenTen_style));
        feature_type_style botEighth_style;
        {
            ...
            {
                polygon_symbolizer poly_sym;
                put(poly_sym, keys::fill, color(99, 223, 220));
       ...
       std::cout << " added 2nd style ... \n";
        ...
        // state data
        {
            ...
            lyr.add_style("nineTen");
            lyr.add_style("tenTen");
